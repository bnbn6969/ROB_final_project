...
template<>
void PublisherSubsrciber<trajectory_msgs::MultiDOFJointTrajectory, nav_msgs::Path>::subcriber_callback(const nav_msgs::Path::ConstPtr& msg){ //收到path訊息
trajectory_msgs::MultiDOFJointTrajectory traj_msg;
traj_msg.header.frame_id="map";(
for (const auto& pose_stamped : msg->poses)   //處理pose_stamped經處理後轉成traj_msg )
  {
    trajectory_msgs::MultiDOFJointTrajectoryPoint point;
    ros::Time start_time = ros::Time::now();
    double time_offset = (pose_stamped.header.stamp - start_time).toSec();
    point.time_from_start = ros::Duration(time_offset);
    geometry_msgs::Transform transform;
    transform.translation.x = pose_stamped.pose.position.x;
    transform.translation.y = pose_stamped.pose.position.y;
    transform.translation.z = pose_stamped.pose.position.z+1;
    transform.rotation.x = pose_stamped.pose.orientation.x;
    transform.rotation.y = pose_stamped.pose.orientation.y;
    transform.rotation.z = pose_stamped.pose.orientation.z;
    transform.rotation.w = pose_stamped.pose.orientation.w;
    point.transforms.push_back(transform);
    traj_msg.points.push_back(point);
  }
publisherObject.publish(traj_msg);    //發佈多自由度軌跡訊息
}
int main(int argc, char** argv) {
  ros::init(argc, argv, "waypoint_publisher_file");   // 節點"waypoint_publisher_file"
  ROS_INFO("Started waypoint_publisher.");
    PublisherSubsrciber<trajectory_msgs::MultiDOFJointTrajectory, nav_msgs::Path> 
  traj_pub_sub ("/firefly/command/trajectory","/path",10 );
  ros::V_string args;
